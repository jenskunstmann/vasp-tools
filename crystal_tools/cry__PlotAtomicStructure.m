function cry__PlotAtomicStructure(crystal, varargin)
...
atomicradius(16) = .6; atomiccolor(16,:) = [1 1 0]; % S
atomicradius(34) = .6; atomiccolor(34,:) = [1 .6 0]; % S
atomicradius(31) = .5; atomiccolor(31,:) = [.5 .5 .5]; % Ga
atomicradius(33) = .5; atomiccolor(33,:) = [1 1 0]; % As
atomicradius(42) = .4; atomiccolor(42,:) = [0 0 1]; % Mo         
%atomicradius(50) = .6; atomiccolor(50,:) = [0 1 1]; % Sn         
atomicradius(74) = .4; atomiccolor(74,:) = [0 0 1]; % W         
atomicradius(82) = .6; atomiccolor(82,:) = [1 0 0]; % Pb         

% plot atom numbers or not
plot_atomID = true;

% plot spheres or symbols
plot_symbols = false;
    ...
    for i2 = cmin(2):cmax(2)
        for i3 = cmin(3):cmax(3)            
            for at = 1:natoms       % loop over the atoms
                ...
                if plot_symbols
                    % plot symbols
                    plot3(pos(1), pos(2), pos(3), 'o', 'Color', color, 'MarkerSize', ceil(rad))
                    ceil(rad)
                else
                    % put the atomic spheres 
                    surf(rad*X0 + pos(1), rad*Y0 + pos(2), rad*Z0 + pos(3), ...
                        'FaceColor',color, 'EdgeColor','none','AmbientStrength',.5);
                end
                
                % plot atomic numbers at their place     
                shi = shift*rad; 
                if plot_atomID
